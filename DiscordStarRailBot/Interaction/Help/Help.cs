...
namespace DiscordStarRailBot.Interaction.Help
{
    [Group("help", "說明")]
        ...
        public async Task H([Summary("模組名稱"), Autocomplete(typeof(HelpGetModulesAutocompleteHandler))] string module = "", [Summary("指令名稱")] string command = "")
        {
            ...
            if (string.IsNullOrWhiteSpace(module))
            {
                ...
                embed.WithDescription(File.ReadAllText(Program.GetDataFilePath("HelpDescription.txt")).Replace("\\n", "\n") +
                    $"\n\n您可以透過 {Format.Url("Patreon", DiscordStarRailBot.Utility.PatreonUrl)} 或 {Format.Url("Paypal", DiscordStarRailBot.Utility.PaypalUrl)} 來贊助 {Context.Client.CurrentUser.Username}");
                await RespondAsync(embed: embed.Build());
                return;
            }
